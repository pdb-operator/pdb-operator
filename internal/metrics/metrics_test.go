...
func TestRecordReconciliation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			count := testutil.CollectAndCount(ReconciliationDuration, "pdb_operator_reconciliation_duration_seconds")
			assert.Greater(t, count, 0, "Reconciliation duration should be recorded")

			if tt.wantError {
				errorCount := testutil.CollectAndCount(ReconciliationErrors, "pdb_operator_reconciliation_errors_total")
...
func TestRecordPDBCreated(t *testing.T) {
	...
	// Check the metric
	count := testutil.CollectAndCount(PDBsCreated, "pdb_operator_pdbs_created_total")
...
func TestUpdateManagedDeployments(t *testing.T) {
	...
	// Check the metric
	count := testutil.CollectAndCount(ManagedDeployments, "pdb_operator_deployments_managed")
...
func TestUpdateActivePoliciesCount(t *testing.T) {
	...
	// Check the metric
	count := testutil.CollectAndCount(AvailabilityPoliciesActive, "pdb_operator_policies_active")
