...
func (acb *AdaptiveCircuitBreakerClient) calculateNewSettings(_, p95, p99 time.Duration, errorRate, throughput float64) *adaptiveSettings {
	settings := &adaptiveSettings{
...
func (acb *AdaptiveCircuitBreakerClient) getBreaker(operation string) *gobreaker.CircuitBreaker[any] {
	acb.mu.RLock()
...
func (acb *AdaptiveCircuitBreakerClient) Apply(ctx context.Context, obj runtime.ApplyConfiguration, opts ...client.ApplyOption) error {
	breaker := acb.getBreaker("apply")

	start := time.Now()
	_, err := breaker.Execute(func() (any, error) {
		return nil, acb.Client.Apply(ctx, obj, opts...)
	})

	acb.recordMetric("apply", time.Since(start), err)
	return err
}
...
func (w *adaptiveStatusWriter) Apply(ctx context.Context, obj runtime.ApplyConfiguration, opts ...client.SubResourceApplyOption) error {
	start := time.Now()
	_, err := w.breaker.Execute(func() (any, error) {
		return nil, w.statusWriter.Apply(ctx, obj, opts...)
	})
	w.recorder("status-apply", time.Since(start), err)
	return err
}
