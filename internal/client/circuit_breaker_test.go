...
func setupTestScheme() *runtime.Scheme {
	...
	_ = pdbv1alpha1.AddToScheme(scheme)
	return scheme
}
...
func TestConcurrentBreakerAccess(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(idx int) {
			name := fmt.Sprintf("concurrent-test-%d", idx%2)
			breaker := acb.getBreaker(name)
	...
	// Should have breakers for the concurrent operations
	acb.mu.RLock()
	_, exists := acb.breakers["concurrent-test-0"]
	acb.mu.RUnlock()
