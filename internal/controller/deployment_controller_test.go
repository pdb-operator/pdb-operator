...
func TestDeploymentReconciler_AnnotationBasedPDB(t *testing.T) {
	...
	assert.Equal(t, "my-component", pdb.Labels[LabelWorkload], "Should have component label")
	assert.Equal(t, "core", pdb.Labels[LabelWorkloadFunction], "Should have function label")
...
func TestDeploymentReconciler_PolicyBasedPDB(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				AnnotationWorkloadName: "webapp-component",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(2),
	...
	// Create an PDBPolicy that matches this deployment
...
func TestDeploymentReconciler_PolicyBasedPDBWithAnnotations(t *testing.T) {
	...
	policy := &pdbv1alpha1.PDBPolicy{
		...
		Spec: pdbv1alpha1.PDBPolicySpec{
			AvailabilityClass: pdbv1alpha1.HighAvailability, // Policy says high-availability
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
				MatchLabels: map[string]string{"tier": "backend"},
			},
			Enforcement: pdbv1alpha1.EnforcementAdvisory, // Advisory mode - annotations preferred
			Priority:    10,
		},
	}
...
func TestDeploymentReconciler_StrictEnforcementMode(t *testing.T) {
	...
	policy := &pdbv1alpha1.PDBPolicy{
		...
		Spec: pdbv1alpha1.PDBPolicySpec{
			AvailabilityClass: pdbv1alpha1.MissionCritical, // Policy enforces mission-critical
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
				MatchLabels: map[string]string{"env": "production"},
			},
			Enforcement: pdbv1alpha1.EnforcementStrict, // Strict - policy always wins
			Priority:    100,
		},
	}
...
func TestDeploymentReconciler_FlexibleEnforcementMode(t *testing.T) {
	...
	policy := &pdbv1alpha1.PDBPolicy{
		...
		Spec: pdbv1alpha1.PDBPolicySpec{
			...
			MinimumClass: pdbv1alpha1.Standard, // Minimum is standard
			Priority:     50,
		},
	}
...
func TestDeploymentReconciler_SecurityFunctionBoost(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationAvailabilityClass: "standard",
				AnnotationWorkloadFunction:  "security", // Security function gets boosted
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(3),
	...
	assert.Equal(t, "security", pdb.Labels[LabelWorkloadFunction], "Should have security function label")
}
...
func TestDeploymentReconciler_PDBOwnershipAndLabels(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationAvailabilityClass: "standard",
				AnnotationWorkloadName:      "test-component",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(3),
	...
	// Verify labels
	assert.Equal(t, OperatorName, pdb.Labels[LabelManagedBy], "Should have managed-by label")
	assert.Equal(t, "test-component", pdb.Labels[LabelWorkload], "Should have component label")
...
func TestDeploymentReconciler_PolicyPriorityResolution(t *testing.T) {
	...
	lowPriorityPolicy := &pdbv1alpha1.PDBPolicy{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "low-priority-policy",
			Namespace: "default",
		},
		Spec: pdbv1alpha1.PDBPolicySpec{
			AvailabilityClass: pdbv1alpha1.NonCritical, // 20%
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
				MatchLabels: map[string]string{"tier": "backend"},
			},
	...
	highPriorityPolicy := &pdbv1alpha1.PDBPolicy{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "high-priority-policy",
			Namespace: "default",
		},
		Spec: pdbv1alpha1.PDBPolicySpec{
			AvailabilityClass: pdbv1alpha1.MissionCritical, // 90%
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
...
func TestDeploymentReconciler_ComponentNameMatching(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				AnnotationWorkloadName: "payment-service",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(3),
	...
	policy := &pdbv1alpha1.PDBPolicy{
		...
		Spec: pdbv1alpha1.PDBPolicySpec{
			AvailabilityClass: pdbv1alpha1.MissionCritical,
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
				WorkloadNames: []string{"payment-service", "billing-service"},
			},
			Enforcement: pdbv1alpha1.EnforcementStrict,
			Priority:    50,
		},
...
func TestDeploymentReconciler_WorkloadFunctionMatching(t *testing.T) {
	ctx := context.Background()
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				AnnotationWorkloadFunction: "management",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(3),
		...
		Spec: pdbv1alpha1.PDBPolicySpec{
			...
			WorkloadSelector: pdbv1alpha1.WorkloadSelector{
				WorkloadFunctions: []pdbv1alpha1.WorkloadFunction{
					pdbv1alpha1.ManagementFunction,
				},
			},
			Enforcement: pdbv1alpha1.EnforcementStrict,
			Priority:    50,
...
func TestDeploymentReconciler_DuplicatePDBCleanup(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationAvailabilityClass: "standard",
				AnnotationWorkloadFunction:  "core",
				AnnotationWorkloadName:      "duplicate-test",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(3),
	...
	pdb1 := &policyv1.PodDisruptionBudget{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"pdboperator.io/managed-by":     "pdb-operator",
				"pdboperator.io/deployment":     "duplicate-pdb-deployment",
				"pdboperator.io/owner-kind":     "Deployment",
				"pdboperator.io/owner-name":     "duplicate-pdb-deployment",
				"pdboperator.io/owner-apigroup": "apps",
			},
		},
		Spec: policyv1.PodDisruptionBudgetSpec{
			MinAvailable: &intstr.IntOrString{Type: intstr.String, StrVal: "50%"},
...
func TestDeploymentReconciler_PDBUpdateOnChange(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationAvailabilityClass: "standard",
				AnnotationWorkloadFunction:  "core",
				AnnotationWorkloadName:      "update-test",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(4),
	...
	oldPdb := &policyv1.PodDisruptionBudget{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"pdboperator.io/managed-by":     "pdb-operator",
				"pdboperator.io/deployment":     "pdb-update-deployment",
				"pdboperator.io/owner-kind":     "Deployment",
				"pdboperator.io/owner-name":     "pdb-update-deployment",
				"pdboperator.io/owner-apigroup": "apps",
			},
			Annotations: map[string]string{
				"pdboperator.io/config-hash": "old-hash",
			},
		},
		Spec: policyv1.PodDisruptionBudgetSpec{
			...
			deployment := &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						AnnotationAvailabilityClass: tc.class,
						AnnotationWorkloadFunction:  "core",
						AnnotationWorkloadName:      "class-test",
					},
				},
				Spec: appsv1.DeploymentSpec{
					Replicas: int32Ptr(4),
