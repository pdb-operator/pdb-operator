...
func TestIsValidAvailabilityClass(t *testing.T) {
	tests := []struct {
		name     string
		class    pdbv1alpha1.AvailabilityClass
		expected bool
	}{
		{"non-critical", pdbv1alpha1.NonCritical, true},
		{"standard", pdbv1alpha1.Standard, true},
		{"high-availability", pdbv1alpha1.HighAvailability, true},
		{"mission-critical", pdbv1alpha1.MissionCritical, true},
		{"custom", pdbv1alpha1.Custom, true},
		{"invalid", pdbv1alpha1.AvailabilityClass("invalid-class"), false},
		{"empty", pdbv1alpha1.AvailabilityClass(""), false},
	}
...
func TestGetDescriptionForClass(t *testing.T) {
	tests := []struct {
		...
		{"non-critical", pdbv1alpha1.NonCritical},
		{"standard", pdbv1alpha1.Standard},
		{"high-availability", pdbv1alpha1.HighAvailability},
		{"mission-critical", pdbv1alpha1.MissionCritical},
		{"custom", pdbv1alpha1.Custom},
	}
...
func TestInferWorkloadFunction(t *testing.T) {
	tests := []struct {
		name           string
		deploymentName string
		labels         map[string]string
		annotations    map[string]string
		expected       pdbv1alpha1.WorkloadFunction
	}{
		{
			name:           "auth in name",
			deploymentName: "auth-service",
			expected:       pdbv1alpha1.SecurityFunction,
		},
		{
		...
		},
		{
			...
			expected:       pdbv1alpha1.ManagementFunction,
		},
		{
		...
		},
		{
			...
			expected:       pdbv1alpha1.CoreFunction,
		},
		{
		...
		},
		{
			...
			annotations:    map[string]string{AnnotationWorkloadFunction: "security"},
			expected:       pdbv1alpha1.SecurityFunction,
		},
		{
			name:           "annotation takes precedence over name",
			deploymentName: "auth-service",
			annotations:    map[string]string{AnnotationWorkloadFunction: "core"},
			expected:       pdbv1alpha1.CoreFunction,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := reconciler.inferWorkloadFunction(deployment)
			assert.Equal(t, tt.expected, result)
		})
	}
}
...
// Tests for PDBPolicy controller helper functions

func TestValidateMaintenanceWindow(t *testing.T) {
	reconciler := &PDBPolicyReconciler{}

	tests := []struct {
		name        string
		window      pdbv1alpha1.MaintenanceWindow
		expectError bool
	}{
		{
			name: "valid window",
...
func TestValidatePolicy(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			policy: &pdbv1alpha1.PDBPolicy{
				Spec: pdbv1alpha1.PDBPolicySpec{
					AvailabilityClass: pdbv1alpha1.AvailabilityClass("invalid"),
					WorkloadSelector: pdbv1alpha1.WorkloadSelector{
						MatchLabels: map[string]string{"app": "test"},
					},
				},
...
func TestUpdateConditions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hasMatches {
				policy.Status.AppliedToWorkloads = []string{"component1"}
			}
...
func TestDeploymentMatchesSelector(t *testing.T) {
	...
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationWorkloadName: "my-component",
			},
		},
	}
	...
	tests := []struct {
		name     string
		selector pdbv1alpha1.WorkloadSelector
		expected bool
	}{
		{
...
func TestGetConfigFromAnnotations(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			hasConfig: true,
			class:     pdbv1alpha1.HighAvailability,
		},
		{
			name:        "no annotations",
			annotations: map[string]string{},
		...
		},
		{
			...
			annotations: map[string]string{
				...
				AnnotationWorkloadFunction:  "security",
			},
			hasConfig: true,
			class:     pdbv1alpha1.Standard,
		},
		{
...
func TestFindMatchingDeployments(t *testing.T) {
	...
	deploy1 := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationWorkloadName: "frontend-component-1",
			},
		},
		Spec: appsv1.DeploymentSpec{
			Replicas: func() *int32 { i := int32(3); return &i }(),
	...
	deploy2 := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				AnnotationWorkloadName: "frontend-component-2",
			},
		},
		Spec: appsv1.DeploymentSpec{
...
func TestResolveConfiguration(t *testing.T) {
	...
	assert.Equal(t, pdbv1alpha1.HighAvailability, result.AvailabilityClass, "Advisory enforcement should prefer annotation")

	// Test strict enforcement - policy should win
	policy.Spec.Enforcement = pdbv1alpha1.EnforcementStrict
	result = reconciler.resolveConfiguration(ctx, deployment, annotationConfig, policyConfig, policy, logger)
	assert.Equal(t, pdbv1alpha1.Standard, result.AvailabilityClass, "Strict enforcement should use policy")

	// Test flexible enforcement with annotation meeting minimum
	policy.Spec.Enforcement = pdbv1alpha1.EnforcementFlexible
	policy.Spec.MinimumClass = pdbv1alpha1.Standard
	result = reconciler.resolveConfiguration(ctx, deployment, annotationConfig, policyConfig, policy, logger)
	assert.Equal(t, pdbv1alpha1.HighAvailability, result.AvailabilityClass, "Flexible enforcement should accept annotation meeting minimum")
	...
	assert.Equal(t, pdbv1alpha1.HighAvailability, result.AvailabilityClass, "No policy should use annotation")

	// Test no annotation, only policy
	result = reconciler.resolveConfiguration(ctx, deployment, nil, policyConfig, policy, logger)
	require.NotNil(t, result)
	assert.Equal(t, pdbv1alpha1.Standard, result.AvailabilityClass, "No annotation should use policy")
}
...
// Test for updateStatus in PDBPolicy controller
	...
	}{
		{
			...
			deployment: &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "default",
					Annotations: map[string]string{
						AnnotationWorkloadName: "component-a",
					},
				},
			},
			policy: &pdbv1alpha1.PDBPolicy{
				...
				Spec: pdbv1alpha1.PDBPolicySpec{
					AvailabilityClass: pdbv1alpha1.Standard,
					WorkloadSelector: pdbv1alpha1.WorkloadSelector{
						WorkloadNames: []string{"component-a", "component-b"},
					},
				},
			},
			expected: true,
		},
		{
			...
			deployment: &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "default",
					Annotations: map[string]string{
						AnnotationWorkloadName: "component-c",
					},
				},
			},
			policy: &pdbv1alpha1.PDBPolicy{
