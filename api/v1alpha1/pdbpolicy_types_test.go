...
func TestWorkloadFunctionConstants(t *testing.T) {
	assert.Equal(t, WorkloadFunction("core"), CoreFunction)
	assert.Equal(t, WorkloadFunction("management"), ManagementFunction)
	assert.Equal(t, WorkloadFunction("security"), SecurityFunction)
}
...
func TestWorkloadSelectorStructure(t *testing.T) {
	selector := WorkloadSelector{
		MatchLabels: map[string]string{
			"app": "test",
		},
		WorkloadNames:     []string{"component-1", "component-2"},
		WorkloadFunctions: []WorkloadFunction{CoreFunction, SecurityFunction},
		Namespaces:        []string{"default", "production"},
	}

	assert.Equal(t, "test", selector.MatchLabels["app"])
	assert.Equal(t, []string{"component-1", "component-2"}, selector.WorkloadNames)
	assert.Equal(t, []WorkloadFunction{CoreFunction, SecurityFunction}, selector.WorkloadFunctions)
	assert.Equal(t, []string{"default", "production"}, selector.Namespaces)
}

func TestPDBPolicySpecStructure(t *testing.T) {
	enforceTrue := true
...
func TestPDBPolicyStatusStructure(t *testing.T) {
	status := PDBPolicyStatus{
		AppliedToWorkloads: []string{"deploy-1", "deploy-2"},
		PDBsManaged:        []string{"pdb-1", "pdb-2"},
		ObservedGeneration: 5,
	}

	assert.Equal(t, []string{"deploy-1", "deploy-2"}, status.AppliedToWorkloads)
