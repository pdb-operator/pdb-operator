...
```bash
# Deploy to pdb-operator-system namespace
kubectl create namespace pdb-operator-system
...
# Enable webhooks
kubectl patch deployment pdb-operator-controller-manager -n pdb-operator-system \
...
metadata:
  ...
  annotations:
    ...
    pdboperator.io/component-function: "core"
spec:
  replicas: 6
...
metadata:
  annotations:
    ...
    pdboperator.io/component-function: "security"

    # Optional: Component name for identification
    pdboperator.io/componentName: "authentication-service"

    # Optional: Maintenance window
    pdboperator.io/maintenance-window: "02:00-04:00 UTC"

    # Optional: Override reason (when required by policy)
    pdboperator.io/override-reason: "Emergency deployment"
```
...
// 1. pdboperator.io/component-function annotation
...
metadata:
  annotations:
    ...
    pdboperator.io/override-reason: "Emergency incident response - approved by SRE team"
...
```bash
# PDB lifecycle
pdb_operator_pdbs_created_total
pdb_operator_pdbs_updated_total
pdb_operator_pdbs_deleted_total

# Performance
pdb_operator_reconciliation_duration_seconds
pdb_operator_cache_hits_total
pdb_operator_cache_misses_total

# Policy enforcement
pdb_operator_enforcement_decisions_total
pdb_operator_override_attempts_total
pdb_operator_policy_conflicts_total

# Operator health
pdb_operator_operator_info
pdb_operator_maintenance_window_active
```
...
```bash
# Enable tracing
kubectl set env deployment/pdb-operator-controller-manager -n pdb-operator-system \
  ENABLE_TRACING=true \
...
kubectl logs -n pdb-operator-system deployment/pdb-operator-controller-manager | grep "resolveConfiguration"
```
...
# Check certificate
kubectl get certificate serving-cert -n pdb-operator-system

# Debug webhook logs
kubectl logs -n pdb-operator-system deployment/pdb-operator-controller-manager | grep webhook
```
...
metadata:
  ...
  annotations:
    pdboperator.io/component-function: "security"
    pdboperator.io/availability-class: "standard" # Ignored due to strict policy
spec:
  replicas: 5
...
metadata:
  ...
  annotations:
    pdboperator.io/availability-class: "mission-critical" # Accepted (higher than minimum)
spec:
  replicas: 10
...
metadata:
  ...
  labels:
    pdboperator.io/componentName: productcatalog
    pdboperator.io/componentType: core
  annotations:
    pdboperator.io/availability-class: "high-availability"
    pdboperator.io/component-function: "core"
    pdboperator.io/api-specification: "TMF620-ProductCatalog-v4.0.0"
    pdboperator.io/maintenance-window: "02:00-04:00 UTC"
spec:
  replicas: 5
...
2. **Port forward to access the MCP server**:
   ...
   ```bash
   kubectl port-forward -n pdb-operator-system service/pdb-operator-mcp-service 8090:8090
   ```
...
- apiGroups: ["availability.pdboperator.io"]
...
- **Namespace Scope**: Policies in the `pdb-operator-system` namespace apply cluster-wide
